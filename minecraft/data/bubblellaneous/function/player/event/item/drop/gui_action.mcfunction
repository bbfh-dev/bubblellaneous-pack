...
# Ignore empty slots
execute as @s[nbt={Item: {components: {"minecraft:custom_data": {CustomModelData: 370000}}}}] run return run kill @s
execute as @s[nbt={Item: {components: {"minecraft:custom_data": {CustomModelData: 370001}}}}] run return run kill @s
execute as @s[nbt={Item: {id: "minecraft:structure_void"}}] run return run kill @s

tag @s add --local.tmp
execute if data entity @s Item.tag.EntityTag.Item.components.minecraft:custom_data.bubblellaneous.block_data as @p[tag=--local.player] run function bubblellaneous:block/bubble_bench/show_docs with entity @e[type=item,tag=--local.tmp,limit=1] Item.components.minecraft:custom_data.minecraft:entity_data.Item.components.minecraft:custom_data.bubblellaneous.block_data
