...
#region Body
$data modify entity @e[type=item_display,tag=local.tmp,limit=1] item.tag.EntityTag.Item.components.minecraft:custom_data.bubblellaneous.block_properties.camera set value {id: $(camera_id), name: '$(name)', location: {dimension: "$(dimension)", x: $(pos_x), y: $(pos_y), z: $(pos_z)}}
#endregion
