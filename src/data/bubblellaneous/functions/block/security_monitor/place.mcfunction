...
execute unless data entity @s item.tag.bubblellaneous.block_properties.camera run function bubblellaneous:block/security_monitor/blockstates/apply/off {rotation: 0}
...
data modify entity @e[type=text_display,tag=--local.new,limit=1,sort=nearest] text set from entity @s item.tag.bubblellaneous.block_properties.camera.name
