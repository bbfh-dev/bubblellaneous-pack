...
data modify storage bubblellaneous tmp.in.name set from entity @p[tag=--local.event_player] SelectedItem.tag.display.Name
...
tellraw @p[tag=--local.event_player] {"translate":"ui.bubblellaneous.camera.registered", "color":"green", "with":[{"nbt":"SelectedItem.tag.display.Name", "entity":"@p[tag=--local.event_player]", "interpret":true}]}
