...
execute if entity @s[tag=-buln.lit] run scoreboard players set $Temp bubblellaneous 1
data modify entity @s Tags set from entity @e[type=item_frame, tag=-buln.temp, limit=1] Tags

execute if score $Temp bubblellaneous matches 1 run tag @s add -buln.lit
