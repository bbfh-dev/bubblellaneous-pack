...
class BlockGroup(BaseEntry):
    ...
    FURNITURE_TYPE = [
        *WOOD_TYPE,
        ...
        ("deepslate", "deepslate", "cut_copper", {"base": "deepslate"}),
                ...
                tree.model(
                    ...
                    Model(
                        json.loads(
                            ...
                            .replace("/primary", f"/{primary}")
                            .replace("/secondary", f"/{secondary}")
                        )
                    ),
                )
        ...
        for i, state in enumerate(blockstates):
            for predicate in state.predicates:
                if predicate is None:
                    code.append(get_blockstate_code("", name, state, None))
                    continue
            ...
            self.make_function(
                ...
                f"function [namespace]:blocks/{name}/blockstate/post_update",
            )
